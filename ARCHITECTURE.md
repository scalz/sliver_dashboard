...
- **Unmodifiable Layout Callbacks (Hardened Invariant):** All callbacks communicating layout state to the outside (`onLayoutChanged`, `onUndo`, `onRedo`, `onWillUndo`, `onWillRedo`) hand out a `List<LayoutItem>.unmodifiable`. All controller mutators centralize their notification via `_notifyLayoutChanged()`. This guarantees that consumers attempting to sort or mutate the list in place throw an immediate, located `UnsupportedError` rather than silently corrupting the controller's internal layout instance or destroying the ascending ID ordering invariant.
...
- **Implementation:**
  ...
  - **Row index (`_RowIndex`):** buckets every item in **each row it spans**, and the buckets are a flat `HashMap` walked by integer over the probe's own rows. A probe therefore walks ~2 rows whatever the layout contains. Bucketing by top row instead (the pre-2.8 shape) made the walk start `maxHeight - 1` rows higher so a tall tile could not be missed — which turned the scan range into a property of the *layout*: one 16-row banner widened every probe in the grid, 2.07 rows/probe against 6.65. Measured on dart2js at N=1000, a cell crossing went from 2.3 ms / 5.5 ms (uniform / banners) to **1.0 ms / 0.9 ms**; at N=4000, from 9.4 ms / 37.6 ms to 4.0 ms / 7.0 ms. The two halves are inseparable: span indexing alone regressed the uniform case ~12 %, and the flat walk alone would have regressed the banner case. A match is emitted once, on the first row of its intersection with the probe. The cascade result is then assembled by walking the input rather than re-sorting it — the id set is unchanged, so the input's own (already ascending) order carries over, and the O(N log N) string sort only runs for an input that was not sorted.
  - **Overlap-Free Invariant:** `moveElement` uses a **monotonic re-push cascade** (items may be re-queued when pushed again, instead of a one-shot `processed` set) followed by an O(N·k) verification pass over that row index. The unconditional O(N²) all-pairs `resolveCollisions` safety net was removed from the per-crossing hot path (499,500 pair checks at N=1000 → ~16,000 indexed checks, 31× fewer). Property (fuzz-tested, 200 seeded dense layouts): **the returned layout contains zero overlapping non-static items.**
...
- **Virtualization:** Only lays out and paints items that are currently visible in the viewport. The window is resolved by a linear scan over the geometry buffer, deliberately: memoizing the buffer across passes and replacing the scan with a binary search over a monotone prefix-maximum of the item ends is *correct* (the raw ends are not monotone, so searching them directly would drop a tall tile out of the child window) but was **measured and rejected** — the two Θ(N) loops together cost 10 µs at N=1000 and 115 µs at N=10000 on dart2js, i.e. 0.06 % and 0.7 % of a 60 Hz frame. See the budget table in `AGENTS.md` §5bis before re-opening this.
...
- **Keep-Alive Scope:** `wantKeepAlive = isResizeGhost || (isDragging && (isSelected || item.moved))`. Keeping tiles alive during a gesture prevents unmount thrash at the cache edge — the cascade shifts hundreds of `y` values per frame, so a pushed tile flickers in and out of the visible+cache window with an unchanged Key and index, and each flicker is a full `Element.inflateWidget`. The scope is what keeps the bucket proportional to the *cascade* rather than to the *scroll distance*: a blanket `isDragging` retains every tile mounted at any point of a long autoscroll drag and releases ~N `State`s in a single `finalizeTree` at drop, whereas this bucket drains incrementally as tiles stop being pushed. The resized tile is included for a different reason — torn down mid-gesture it would take its `FocusNode` with it (see the Item Persistence rule).
...
- **The background is the FIRST slot of the overlay's `Stack` when it exists at
  all; the `CustomScrollView` is the slot after it.** The slot is conditional —
  it is emitted only for a `backgroundBuilder` or a non-null `gridStyle` — which
  is why every slot of that `Stack` carries a key (see §8). On the frame both are
  first built, no `RenderSliverDashboard` exists
...
| Scroll offset resets — and `ScrollController attached to multiple scroll views` is thrown from `DashboardGrid` — every time edit mode is toggled | the overlay `Stack` had UNKEYED children and a conditional background slot (`gridStyle: isEditing ? … : null`). Inserting it shifted the content slot by one index; both are `Positioned`, so `Element.updateChildren`'s descending pass matched them and the host `CustomScrollView` was torn down instead of the insertion being recognised — two `ScrollPosition`s on one controller for the remount frame. Reported from a consumer (`linksys/PrivacyGUI#1534`), never reachable from `Dashboard` / `NestedDashboard`, whose `gridStyle` is never null, so the suite could not see it | one key per Stack slot (§3C invariant); "Overlay Stack slot stability" tests toggling `gridStyle` and `backgroundBuilder` in both directions, counting MOUNTS — a `find` cannot see a remount |
| Pressing `Delete` removes the previously focused tile instead of the clicked one; two tiles visually wear the border | `_onPointerDown` updated `selectedItemIds` without transferring Flutter's keyboard focus. The primary focus stayed on the previous tile, which received the shortcut and executed the deletion intent on itself. | `DashboardItem` owns its `FocusNode` and claims focus on pointer-down (leaving selection to the overlay); `_focusClaimedPointer` arbitrates deepest-first dispatch for nested grids; focus release is gated on the `_wasSelected` transition to empty. |
| A drag inside a nested grid does nothing: the tile goes invisible behind its own feedback shadow and stays there; in debug, `This widget has been unmounted, so the State no longer has a context` from `_DashboardOverlayState._findRenderSliver`. Works on the next attempt | the tile's chrome was `Container(decoration: <nullable>)`, which emits a `DecoratedBox` **and** the border's padding only when the decoration is non-null. Any selection / focus / displaced / nest-hover transition therefore RE-PARENTED the cached content — destroying the application subtree inside the tile, including a `NestedDashboard`'s whole `DashboardOverlay`. `GestureBinding` freezes the hit-test path at pointer-down and replays it, so the rest of the gesture ran on the dead `State`. Reachable when the host tile holds the keyboard focus and the press hands it to a nested tile — exactly what returning to the window and clicking inside a folder does | the chrome layers are unconditional: `DecoratedBox(decoration ?? const BoxDecoration())` over `Padding(decoration?.padding ?? EdgeInsets.zero)` (§3C invariant); `debugOverlayDisposedDuringInteraction` + `test/view/nested/nested_drag_survival_test.dart`, which counts MOUNTS inside the tile and asserts a HORIZONTAL displacement (vertical compaction makes a `y` assertion pass for the wrong reason) |_